...
class WithExitCallNode(ExprNode):
    ...
    def analyse_types(self, env):
        ...
        if self.aawait:
            self.aawait = self.aawait.analyse_types(env)
    ...
    def generate_evaluation_code(self, code):
        ...
        if self.aawait:
            ...
            self.aawait.generate_evaluation_code(code, source_cname=result_var, decref_source=True)
            code.putln("%s = %s;" % (result_var, self.aawait.py_result()))
            self.aawait.generate_post_assignment_code(code)
            self.aawait.free_temps(code)
